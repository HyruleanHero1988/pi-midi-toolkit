...
TOUCH_SCROLL_THRESH_PX = 14  # press→drag before a voice-grid tap counts as scroll
...
# Screen grid (top→bottom) matching the physical MPK pad layout
DRUM_GRID_NOTES: Tuple[int, ...] = (
    40, 41, 42, 43,  # Bank A top
    36, 37, 38, 39,  # Bank A bottom
    48, 49, 50, 51,  # Bank B top
    44, 45, 46, 47,  # Bank B bottom
)

DRUM_SHORT: Dict[str, str] = {
    "kick": "KICK",
    "snare": "SNR",
    "clap": "CLAP",
    "hat_closed": "CHH",
...
def mpk_pad_label(note: int) -> str:
    n = note & 0x7F
    if 36 <= n <= 43:
        return f"A{n - 35}"
    if 44 <= n <= 51:
...
def drum_button_label(note: int) -> str:
    model = drum_model_for_note(note)
    short = DRUM_SHORT.get(model, model.upper()[:6])
    return f"{mpk_pad_label(note)}\n{short}"


@dataclass
class DrumParams:
    """Per-pad synth macros (0..1). Defaults match the old global kit."""
    pitch: float = 0.45
    decay: float = 0.40  # stretch / body
    noise: float = 0.55
    tone: float = 0.60

    def copy(self) -> "DrumParams":
        return DrumParams(self.pitch, self.decay, self.noise, self.tone)
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "DrumParams":
        p = cls()
        if "pitch" in data:
            p.pitch = max(0.0, min(1.0, float(data["pitch"])))
        if "decay" in data:
            p.decay = max(0.0, min(1.0, float(data["decay"])))
        if "noise" in data:
            p.noise = max(0.0, min(1.0, float(data["noise"])))
        if "tone" in data:
            p.tone = max(0.0, min(1.0, float(data["tone"])))
        return p
...
class SineEngine:
    ...
    def __init__(
        ...
        # Per-pad macros (note → params). Knobs edit the selected pad only.
        self._drum_params: Dict[int, DrumParams] = {
            n: DrumParams() for n in MPK_PAD_KIT
        }
        self._drum_selected: Optional[int] = 36  # default: Bank A pad 1 (kick)
        self._drum_focus_until = 0.0
        self._drum_edit_mode = False  # True while DRUMS UI is open
    ...
    def _drum_note_on(self, note: int, velocity: float) -> None:
        with self._lock:
            ...
            p = self._params_for_unlocked(note)
    ...
    def _params_for_unlocked(self, note: int) -> DrumParams:
        note = note & 0x7F
        p = self._drum_params.get(note)
        if p is None:
            p = DrumParams()
            self._drum_params[note] = p
        return p

    def select_drum(self, note: int, *, keep_focus: bool = True) -> None:
        """Arm a pad for knob editing (and optionally extend drum-knob focus)."""
        with self._lock:
            self._drum_selected = note & 0x7F
            if keep_focus:
                self._drum_focus_until = time.monotonic() + self.DRUM_FOCUS_SEC

    def selected_drum(self) -> Optional[int]:
        with self._lock:
            return self._drum_selected

    def drum_params(self, note: Optional[int] = None) -> DrumParams:
        with self._lock:
            n = self._drum_selected if note is None else (note & 0x7F)
            if n is None:
                return DrumParams()
            return self._params_for_unlocked(n).copy()

    def set_drum_edit_mode(self, enabled: bool) -> None:
        """While DRUMS UI is open, knobs stay on the selected pad."""
        with self._lock:
            self._drum_edit_mode = bool(enabled)
            if self._drum_edit_mode:
                self._drum_focus_until = time.monotonic() + 3600.0

    def drum_edit_mode(self) -> bool:
        with self._lock:
            return self._drum_edit_mode
    ...
    def drum_knob_focus(self) -> bool:
        """True when knobs should edit the selected pad (DRUMS UI or recent hit)."""
        with self._lock:
            if self._drum_edit_mode:
                return True
    ...
    def _set_selected_param(self, attr: str, value: float) -> None:
        if value > 1.0:
            value = value / 127.0
        value = max(0.0, min(1.0, float(value)))
        with self._lock:
            note = self._drum_selected
            if note is None:
                note = 36
                self._drum_selected = note
            p = self._params_for_unlocked(note)
            setattr(p, attr, value)
            if not self._drum_edit_mode:
                self._drum_focus_until = time.monotonic() + self.DRUM_FOCUS_SEC

    def set_drum_pitch(self, value: float) -> None:
        self._set_selected_param("pitch", value)

    def set_drum_decay(self, value: float) -> None:
        """Stretch / body length for the selected pad."""
        self._set_selected_param("decay", value)

    def set_drum_noise(self, value: float) -> None:
        self._set_selected_param("noise", value)

    def set_drum_tone(self, value: float) -> None:
        self._set_selected_param("tone", value)

    def reset_drum_params(self, note: Optional[int] = None) -> None:
        with self._lock:
            n = self._drum_selected if note is None else (note & 0x7F)
            if n is None:
                return
            self._drum_params[n] = DrumParams()
            self._drum_selected = n
    ...
    def modulation_state(self) -> Dict[str, float]:
        with self._lock:
            note = self._drum_selected if self._drum_selected is not None else 36
            p = self._params_for_unlocked(note)
            return {
                ...
                "drum_noise": p.noise,
                "drum_tone": p.tone,
                "drum_selected": float(note),
                "drum_edit": 1.0 if self._drum_edit_mode else 0.0,
            }

    def snapshot_settings(self) -> Dict[str, Any]:
        """Serialize synth sound settings for JSON presets / session restore."""
        with self._lock:
            kit = {str(n): params.as_dict() for n, params in self._drum_params.items()}
            sel = self._drum_selected if self._drum_selected is not None else 36
            p = self._params_for_unlocked(sel)
            return {
                ...
                # Selected-pad snapshot (compat with older single-kit fields)
                "drum_pitch": float(p.pitch),
                "drum_decay": float(p.decay),
                "drum_noise": float(p.noise),
                "drum_tone": float(p.tone),
                "drum_selected": int(sel),
                "drum_kit": kit,
            }
    ...
    def apply_settings(self, data: Dict[str, Any]) -> None:
        ...
        with self._lock:
            ...
            kit = data.get("drum_kit")
            if isinstance(kit, dict):
                for key, pdata in kit.items():
                    try:
                        note = int(key)
                    except (TypeError, ValueError):
                        continue
                    if isinstance(pdata, dict):
                        self._drum_params[note & 0x7F] = DrumParams.from_dict(pdata)
            else:
                # Legacy: one global kit → apply to every pad
                legacy = DrumParams()
                if "drum_pitch" in data:
                    legacy.pitch = max(0.0, min(1.0, float(data["drum_pitch"])))
                if "drum_decay" in data:
                    legacy.decay = max(0.0, min(1.0, float(data["drum_decay"])))
                if "drum_noise" in data:
                    legacy.noise = max(0.0, min(1.0, float(data["drum_noise"])))
                if "drum_tone" in data:
                    legacy.tone = max(0.0, min(1.0, float(data["drum_tone"])))
                if any(k in data for k in ("drum_pitch", "drum_decay", "drum_noise", "drum_tone")):
                    for n in list(self._drum_params.keys()) or list(MPK_PAD_KIT.keys()):
                        self._drum_params[n] = legacy.copy()

            if "drum_selected" in data:
                try:
                    self._drum_selected = int(data["drum_selected"]) & 0x7F
                except (TypeError, ValueError):
                    pass
...
class MidiToneApp:
    def __init__(
        ...
        self._mode = "synth"  # synth | drums | looper | pads | songs | log | presets
        ...
        self._drums_shell = tk.Frame(self._mode_host, bg="#111111")
        ...
        self._drum_pad_btns: Dict[int, tk.Button] = {}
        self._drum_sel_var = tk.StringVar(value="Select a pad")
        self._drum_params_var = tk.StringVar(value="")
        ...
        self._mk_touch_btn(row3, "DRUMS", lambda: self._switch_mode("drums"), bg="#d79921").pack(
            side=tk.LEFT, expand=True, fill=tk.BOTH, padx=3, ipady=8
        )
        ...
        self._append_log(
            "DRUMS mode: tap a pad (or hit MPK), then knobs edit THAT pad only — "
            "pitch / stretch / noise / tone / level"
        )
    ...
    def _format_mod_line(self) -> str:
        ...
        if self.engine.drum_knob_focus():
            note = int(st.get("drum_selected", 36))
            model = drum_model_for_note(note)
            short = DRUM_SHORT.get(model, model.upper()[:6])
            mode = "EDIT" if st.get("drum_edit", 0) else "auto"
            return (
                f"DRUM[{mode}] {mpk_pad_label(note)} {short}  "
    ...
    def _build_drums_mode(self) -> None:
        shell = self._drums_shell
        for w in shell.winfo_children():
            w.destroy()
        ...
        header.pack(fill=tk.X, padx=8, pady=(8, 2))
        tk.Label(
            header, text="Drums", font=("DejaVu Sans", 18, "bold"),
            fg="#fbf1c7", bg="#111111",
        ).pack(side=tk.LEFT)
        tk.Label(
            header, text="tap pad · knobs edit that pad only",
            font=("DejaVu Sans", 11), fg="#a89984", bg="#111111",
        ).pack(side=tk.RIGHT)

        sel = tk.Label(
            shell, textvariable=self._drum_sel_var,
            font=("DejaVu Sans", 15, "bold"),
            fg="#fabd2f", bg="#111111", anchor="w",
        )
        sel.pack(fill=tk.X, padx=10, pady=(4, 0))
        params = tk.Label(
            shell, textvariable=self._drum_params_var,
            font=("DejaVu Sans Mono", 13, "bold"),
            fg="#8ec07c", bg="#111111", anchor="w",
        )
        params.pack(fill=tk.X, padx=10, pady=(0, 4))

        grid = tk.Frame(shell, bg="#111111")
        grid.pack(fill=tk.BOTH, expand=True, padx=6, pady=2)
        self._drum_pad_btns = {}
        for i, note in enumerate(DRUM_GRID_NOTES):
            r, c = divmod(i, 4)
            btn = self._mk_touch_btn(
                grid,
                drum_button_label(note),
                lambda n=note: self._drums_select_pad(n, preview=True),
                bg="#3c3836",
            )
            btn.configure(font=("DejaVu Sans", 11, "bold"), pady=8, justify=tk.CENTER)
            btn.grid(row=r, column=c, sticky="nsew", padx=3, pady=3, ipady=6)
            self._drum_pad_btns[note] = btn
        for c in range(4):
            grid.grid_columnconfigure(c, weight=1)
        for r in range(4):
            grid.grid_rowconfigure(r, weight=1)
        ...
        footer.pack(fill=tk.X, padx=8, pady=8)
        self._mk_touch_btn(
            footer, "RESET PAD", self._drums_reset_selected, bg="#504945"
        ).pack(side=tk.LEFT, expand=True, fill=tk.BOTH, padx=3, ipady=12)
        self._mk_touch_btn(
            footer, "BACK TO SYNTH", lambda: self._switch_mode("synth"), bg="#458588"
        ).pack(side=tk.LEFT, expand=True, fill=tk.BOTH, padx=3, ipady=12)

    def _drums_select_pad(self, note: int, *, preview: bool = False) -> None:
        self.engine.select_drum(note)
        if preview:
            # Audition with current per-pad macros
            self.engine.note_on(DRUM_CHANNEL, note, 100)
            self.root.after(80, lambda n=note: self.engine.note_off(DRUM_CHANNEL, n))
        self._paint_drums_ui()
        self.mod_var.set(self._format_mod_line())
        self.last_var.set(f"Drum edit → {mpk_pad_label(note)} {drum_model_for_note(note)}")
        self._q_put(("log", f"Drum selected → {mpk_pad_label(note)} {drum_model_for_note(note)}", False))

    def _drums_reset_selected(self) -> None:
        note = self.engine.selected_drum()
        if note is None:
            return
        self.engine.reset_drum_params(note)
        self._mark_settings_dirty()
        self._paint_drums_ui()
        self.mod_var.set(self._format_mod_line())
        self._append_log(f"Reset macros → {mpk_pad_label(note)}")

    def _paint_drums_ui(self) -> None:
        note = self.engine.selected_drum()
        if note is None:
            self._drum_sel_var.set("Select a pad")
            self._drum_params_var.set("Knobs: Pitch · Stretch · Noise · Tone · Level")
        else:
            model = drum_model_for_note(note)
            short = DRUM_SHORT.get(model, model.upper())
            self._drum_sel_var.set(f"Editing  {mpk_pad_label(note)}  ·  {short}  (note {note})")
            p = self.engine.drum_params(note)
            self._drum_params_var.set(
                f"Pitch {int(p.pitch * 127):3d}   "
                f"Stretch {int(p.decay * 127):3d}   "
                f"Noise {int(p.noise * 127):3d}   "
                f"Tone {int(p.tone * 127):3d}"
            )
        selected = note
        for n, btn in self._drum_pad_btns.items():
            on = selected is not None and n == selected
    ...
    def _switch_mode(self, mode: str) -> None:
        mode = mode if mode in ("synth", "drums", "looper", "pads", "songs", "log", "presets") else "synth"
        ...
        # Knobs stay on the selected pad only while DRUMS mode is open
        self.engine.set_drum_edit_mode(mode == "drums")
        if prev == "drums" and mode != "drums":
            self.engine.touch_drum_focus()  # brief auto window after leaving

        if mode == "drums":
            self._drums_shell.pack(fill=tk.BOTH, expand=True)
            self._paint_drums_ui()
            self.mod_var.set(self._format_mod_line())
        elif mode == "looper":
    ...
    def _open_voice_grid(self) -> None:
        ...
        # Pack CLOSE first (bottom) so the scroll area never pushes it off-screen
        ...
        for i, name in enumerate(self._voice_names):
            ...
            # Compact enough that a few rows fit; rail scrolls the rest
    ...
    def _handle_knob_cc(self, control: int, value: int) -> Optional[str]:
        ...
        # After a pad hit (or while DRUMS mode is open), knobs edit THAT pad only
        if self.engine.drum_knob_focus():
            if control == CC_MORPH:
