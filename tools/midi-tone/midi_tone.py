...
def drum_model_for_note(note: int) -> str:
    """Map MIDI note (ch10 / GM-ish) → procedural drum model."""
    n = note & 0x7F
    if n in (35, 36):
        return "kick"
    if n in (37, 39):
    ...
    if n in (38, 40):
        return "snare"
    if n in (41, 43, 45, 47, 48, 50):
        return "tom"
    if n in (42, 44, 46, 49, 51, 52, 55, 57, 59):
        return "hat"
    if n < 38:
        return "kick"
    if n < 42:
...
class SineEngine:
    ...
    def note_off(self, channel: int, note: int) -> None:
        ...
        with self._lock:
            if (channel & 0x0F) == DRUM_CHANNEL:
                ...
                # One-shots keep decaying; open-hat-style: release shortens remaining
                hit = self._drums.get(key[1])
                if hit is not None and hit.model == "hat":
                    hit.decay *= 0.35
                return
    ...
    def _render_drums(
        ...
        for hit in hits:
            ...
            # Shared noise (moving-average “LP”; higher tone → shorter / brighter)
            ...
            if model == "kick":
                f0 = 48.0 * (2.0 ** ((pitch - 0.5) * 1.8))
                f_end = 28.0 + 18.0 * pitch
                drop_tau = 0.015 + 0.05 * (1.0 - decay)
                body_tau = 0.06 + 0.42 * decay
                freq = f_end + (f0 - f_end) * np.exp(-t / np.float32(drop_tau))
                phase_inc = two_pi * freq * inv_sr
                phases = hit.phase + np.cumsum(phase_inc)
                hit.phase = float(phases[-1] % two_pi)
                env = np.exp(-t / np.float32(body_tau))
                click = np.exp(-t / np.float32(0.004)) * np.float32(0.22 * vel)
                body = np.sin(phases) * env * np.float32(0.38 * vel)
                # Tiny noise tick
                sig = body + click * white + noise * np.float32(0.06 * noise_amt * vel) * env
                dur = body_tau * 4.5
            elif model == "tom":
                f0 = 90.0 * (2.0 ** ((pitch - 0.5) * 2.2 + (hit.note % 12) * 0.04))
                f_end = 55.0 + 40.0 * pitch
                drop_tau = 0.02 + 0.06 * (1.0 - decay)
                body_tau = 0.05 + 0.35 * decay
                freq = f_end + (f0 - f_end) * np.exp(-t / np.float32(drop_tau))
                phase_inc = two_pi * freq * inv_sr
                phases = hit.phase + np.cumsum(phase_inc)
                hit.phase = float(phases[-1] % two_pi)
                env = np.exp(-t / np.float32(body_tau))
                sig = np.sin(phases) * env * np.float32(0.32 * vel)
                sig += noise * np.float32(0.10 * noise_amt * vel) * env
                dur = body_tau * 4.5
            elif model == "snare":
                f0 = 180.0 * (2.0 ** ((pitch - 0.5) * 1.4))
                body_tau = 0.03 + 0.18 * decay
                noise_tau = 0.04 + 0.28 * decay
                phase_inc = two_pi * f0 * inv_sr
                phases = hit.phase + phase_inc * (arange + 1.0)
                hit.phase = float(phases[-1] % two_pi)
                tone_env = np.exp(-t / np.float32(body_tau))
                noise_env = np.exp(-t / np.float32(noise_tau))
                sig = np.sin(phases) * tone_env * np.float32(0.16 * vel)
                sig += noise * noise_env * np.float32((0.18 + 0.40 * noise_amt) * vel)
                dur = max(body_tau, noise_tau) * 5.0
            elif model == "clap":
                # A few early bursts + noise tail
                ...
                open_hat = hit.note in (46, 49, 57)
                noise_tau = (0.04 + 0.35 * decay) if open_hat else (0.015 + 0.08 * decay)
                # Brighter: highpass ≈ white - lp
                bright = white - noise * np.float32(0.85)
                noise_env = np.exp(-t / np.float32(noise_tau))
                sig = bright * noise_env * np.float32((0.14 + 0.32 * noise_amt) * vel)
                # tone darkens by mixing more LP
                sig = sig * np.float32(0.35 + 0.65 * tone) + noise * noise_env * np.float32(
                    0.12 * (1.0 - tone) * vel
                )
                dur = noise_tau * 5.5

            buf += sig.astype(np.float32, copy=False)
            hit.pos += frames
            if hit.pos > int(dur * sr) or float(np.max(np.abs(sig))) < 0.0002:
    ...
    def _handle_midi(self, msg: mido.Message) -> None:
        ...
        if msg.type == "note_on" and msg.velocity > 0:
            ...
            if is_drum:
                ...
                line = (
                    f"Pad/{model:5s} ch{msg.channel + 1}  {midi_note_name(msg.note)} "
