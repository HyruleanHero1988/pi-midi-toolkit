...
chmod +x "$DIR/kiosk.sh" "$DIR/run.sh" "$DIR/launch-desktop.sh" \
  "$DIR/install-kiosk.sh" "$DIR/disable-kiosk.sh" "$DIR/hide-touch-cursor.sh" 2>/dev/null || true
sed -i 's/\r$//' "$DIR/kiosk.sh" "$DIR/install-kiosk.sh" "$DIR/disable-kiosk.sh" \
  "$DIR/hide-touch-cursor.sh" "$DIR/kiosk/openbox/autostart" "$DIR/kiosk/openbox/rc.xml" \
