...
def engine_binary() -> Optional[pathlib.Path]:
    ...
    # Package lives at <deploy>/pidi/; deploy root is apps/pidi or ~/midi-tone.
    deploy = here.parent if here.name == "pidi" else here
    roots: List[pathlib.Path] = []
    # Full clone: apps/pidi → repo root.
    if deploy.name == "pidi" and deploy.parent.name == "apps":
        roots.append(deploy.parents[1])
    # Legacy clone path tools/midi-tone.
    if deploy.name == "midi-tone" and deploy.parent.name == "tools":
        roots.append(deploy.parents[1])
    roots.append(deploy)
