#!/usr/bin/env python3
"""Opt-in software update for the Pi MIDI box (called by pidi-native).

This is a deploy helper, not the retired Tk kiosk. The native UI shells
out to ``deploy/updater.py`` for CHECK/INSTALL.
...
* kiosk assets (``apps/pidi``)
* crates, deploy scripts, shipped presets
* restart ``midi-engine`` / ``jambox-engine`` when those units exist
* never ``systemctl stop`` / ``pkill`` ``pidi-native`` (that blanks KMSDRM
  and kills the installer). The kiosk re-execs itself after ``RELOAD_KIOSK=1``.
...
def _install_and_repo() -> tuple[pathlib.Path, pathlib.Path]:
    """Kiosk asset dir + git/overlay root.

    Canonical path is ``deploy/updater.py``. A leftover
    ``apps/pidi/pidi/updater.py`` on a live Pi still resolves correctly.
    """
    script = pathlib.Path(__file__).resolve()
    if script.parent.name == "deploy":
        repo = script.parent.parent
        return repo / MIDI_TONE_REL, repo
    install = script.parents[1]
    repo = script.parents[2] if len(script.parents) >= 3 else install
    return install, repo


HERE, SCRIPT_REPO = _install_and_repo()
...
def looks_like_repo_root(path: pathlib.Path) -> bool:
    """True for the native tree (crates / staged bins) or the old Tk layout."""
    if not (path / "Cargo.toml").is_file():
        return False
    return (
        (path / "crates" / "pidi-native").is_dir()
        or (path / STAGED_BIN_DIR / "pidi-native").is_file()
        or (path / STAGED_BIN_DIR / "jambox-engine").is_file()
        or (path / MIDI_TONE_REL / "pidi" / "updater.py").is_file()
        or (path / MIDI_TONE_REL / "midi_tone.py").is_file()
        or (path / MIDI_TONE_REL).is_dir()
    )
...
def repo_root_for(install: pathlib.Path = HERE) -> pathlib.Path:
    ...
    env = (
        os.environ.get("MIDI_TONE_REPO_ROOT", "").strip()
        or os.environ.get("PIDI_REPO_ROOT", "").strip()
    )
    if env:
        return pathlib.Path(env).expanduser().resolve()
    if looks_like_repo_root(SCRIPT_REPO):
        return SCRIPT_REPO
...
# Audio engines may be stopped for a binary swap. The kiosk must stay up.
_STOPPABLE_ENGINE_BINS = ("midi-engine", "jambox-engine")


def _stop_engines(progress: ProgressCb = _noop_progress) -> None:
    """Stop audio engines so ``bin/`` can be replaced without ETXTBSY.

    Never stops ``pidi-native``: that blanks KMSDRM and SIGKILLs this
    installer when systemd tears down the kiosk cgroup.
    """
    if os.name == "nt":
        return
    stopped = False
    for unit in _STOPPABLE_ENGINE_BINS:
        if not _engine_unit_enabled(unit):
            continue
        progress(f"Stopping {unit}…")
        _systemctl("stop", unit)
        stopped = True
    for name in _STOPPABLE_ENGINE_BINS:
        try:
...
def _restart_engines(progress: ProgressCb) -> None:
    """Restart audio engine units. The kiosk re-execs itself if replaced."""
    for unit in _STOPPABLE_ENGINE_BINS:
        if not _engine_unit_enabled(unit):
            continue
...
def _self_test() -> int:
    """Identity + kiosk-safety checks (no network, no device)."""
    with tempfile.TemporaryDirectory(prefix="pidi-updater-self-") as tmp:
        root = pathlib.Path(tmp)
        native = root / "native"
        (native / "crates" / "pidi-native").mkdir(parents=True)
        (native / "Cargo.toml").write_text("[package]\nname='x'\n", encoding="utf-8")
        tk = root / "tk"
        (tk / MIDI_TONE_REL).mkdir(parents=True)
        (tk / "Cargo.toml").write_text("[package]\nname='x'\n", encoding="utf-8")
        (tk / MIDI_TONE_REL / "midi_tone.py").write_text("# leftover\n", encoding="utf-8")
        empty = root / "empty"
        empty.mkdir()
        (empty / "Cargo.toml").write_text("[package]\nname='x'\n", encoding="utf-8")
        assert looks_like_repo_root(native), "native tree (crates/pidi-native) must be accepted"
        assert looks_like_repo_root(tk), "legacy Tk tree must still be accepted"
        assert not looks_like_repo_root(empty), "Cargo.toml alone is not a repo"
        assert not looks_like_repo_root(root), "temp parent is not a repo"
        found = _find_repo_root(root)
        assert found == native or found == tk
    assert "pidi-native" not in _STOPPABLE_ENGINE_BINS
    print("self-test ok: native tree identity + kiosk not in stop list")
    return 0
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    parser.add_argument(
        "--self-test",
        action="store_true",
        help="check native-tree identity without touching the device",
    )
    ...
    if args.apply:
        ...
        try:
            ...
            print(f"UpdateError: {exc}", file=sys.stderr, flush=True)
            return 1
